...
def edit_player_details(request, user_id):
    ...
    event_registered = request.form.get('events')
    event_registered = json.loads(event_registered)
    print(event_registered, list(event_registered.keys()))
    # for e in event_registered:
    #     print(e)

    events = Event.query.all()
    event_list = []
    for event in events:
        gender_allowed_list = [g for g in event.gender_allowed.split(',')]
